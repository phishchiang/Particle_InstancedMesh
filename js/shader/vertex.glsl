...
attribute vec3 instance_position; // Global position of the mesh, random scattering (posi)
attribute vec3 each_instance_position; // (positioin)
uniform float time;
uniform vec2 pixels;
...
vec3 fbm_vec3(vec3 p, float frequency, float offset){
  return vec3(
    cnoise(p + vec3(offset + 0.0) * frequency),
...
void main() {
  vUv = each_instance_position.xy + vec2(0.5);

  float particle_size = cnoise(instance_position * 2.5) * 0.5 + 0.5; // particle size from cnoise

  vec3 world_position = rotate(instance_position, vec3(0.0, 1.0, 0.0), time * 0.2 * (0.1 + 0.85 * particle_size )); // particle rotation

  vec3 offset = getOffset(world_position);
  ...
  // view_position.xyz = view_position.xyz + each_instance_position *( 0.01 + 0.1 * particle_size);
